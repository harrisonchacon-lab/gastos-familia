...
RADIO          = [14]   # esquinas redondeadas globales
...
def hacer_input(hint):
    ...
    # TextInput limpio adentro — sin canvas propio para no bloquear toques
    ...
    # Exponemos el TextInput como atributo para acceder a .text desde afuera
...
class PantallaResumen(Screen):
    ...
    def construir(self, filtro_mes="Todos"):
        ...
        if es_filtrado:
            bloque_ant = BoxLayout(orientation="horizontal",
                                   size_hint_y=None, height=44, padding=[12,6])
            tarjeta_redondeada(bloque_ant)
            c_ant = COLOR_VERDE if saldo_ant >= 0 else COLOR_ROJO
            bloque_ant.add_widget(Label(text="Saldo arrastrado:", font_size=12,
                                        color=COLOR_GRIS, halign="left",
            ...
            raiz.add_widget(hacer_label(f"Movimientos de {filtro_mes[3:]}:",
                                        tamanio=12, color=COLOR_GRIS))
            ...
            bloque_acum = BoxLayout(orientation="horizontal",
                                    size_hint_y=None, height=50, padding=[12,6])
        ...
        raiz.add_widget(hacer_label(f"Movimientos: {len(movs)}",
                                    tamanio=12, color=COLOR_GRIS, halign="center"))
        raiz.add_widget(hacer_label("Editar  /  Borrar cada movimiento con los botones:",
                                    tamanio=11, color=COLOR_GRIS))
        ...
        if not movs:
            lista.add_widget(hacer_label("No hay movimientos para este periodo.",
                                         color=COLOR_GRIS, halign="center"))
    ...
    def _fila(self, row, filtro_mes):
        ...
        fila = BoxLayout(orientation="horizontal",
                         size_hint_y=None, height=58, spacing=5, padding=[8,5])
        ...
        col.add_widget(Label(text=desc, font_size=12, color=COLOR_TEXTO,
                             halign="left", text_size=(None,None)))
        col.add_widget(Label(text=cat,  font_size=10, color=color_cat,
                             halign="left", text_size=(None,None)))

        lbl_monto = Label(text=f"${monto:,.0f}", font_size=13,
        ...
        with btn_editar.canvas.before:
            Color(*COLOR_EDITAR)
            btn_editar._rect = RoundedRectangle(pos=btn_editar.pos,
                                                size=btn_editar.size, radius=[8])
        ...
        with btn_borrar.canvas.before:
            Color(*COLOR_BORRAR)
            btn_borrar._rect = RoundedRectangle(pos=btn_borrar.pos,
                                                size=btn_borrar.size, radius=[8])
...
class PantallaCategorias(Screen):
    ...
    def _barra(self, categoria, total, color):
        fila = BoxLayout(orientation="horizontal",
                         size_hint_y=None, height=44, spacing=8, padding=[12,4])
        tarjeta_redondeada(fila)
